...
static inline void
ps_list_ll_add_all(struct ps_list *dst, struct ps_list *h, struct ps_list *t)
{
	t->n = dst->n;
	dst->n = h->n;
	h->n->p = dst;
...
#define ps_list_head_append_all(lh1, lh2)        ps_list_ll_add_all(((&(lh1)->l)->p), (&(lh2)->l), ((&(lh2)->l)->p))
...
#define ps_list_head_append_all_d(lh1, lh2)   ps_list_head_append_all(lh1, lh2)
