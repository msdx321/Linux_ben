...
*/
struct mynode * my_search(struct rb_root *root, unsigned long long deadline)
{
  	...
  	while (node) {
		...
		if (deadline < data->deadline)
  			node = node->rb_left;
		else if (deadline > data->deadline)
  			node = node->rb_right;
		else
  			return data;
	}
...
struct mynode * my_min(struct rb_root *root)
{
...
int main()
{
	...
	for (; i < NUM_NODES; i++) {
		...
		data = my_search(&mytree, mn[i]->string);
		printf("data: %p, mn: %p\n", data, mn[i]);
		printf("data: %s, mn: %s\n", data->string, mn[i]->string);
	}
